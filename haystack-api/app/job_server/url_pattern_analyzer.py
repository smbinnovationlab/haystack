...
from analyzer import product_price_analyzer_2nd
...
channel.queue_declare(queue=config.MQ_QUEUE_2ND, durable=True)
print(' [*] Waiting for messages. To exit press CTRL+C')


def callback(ch, method, properties, body):
    request_msg = json.loads(body)
    request_msg['debug_info']['time_cost']['url_pattern'] = float(
        request_msg.get('debug_info').get('time_cost').get('url_pattern')
    )
    ...
    if int(request_msg.get('retry_times')) < 3:
        try:
            ret_flag, t1, t2 = product_price_analyzer_2nd.get_price(site_id=request_msg.get('site_id'),
                                                                    site_data=request_msg,
                                                                    need_screenshot=False)

            request_msg['debug_info']['time_cost']['url_pattern'] += t1
            request_msg['debug_info']['time_cost']['screenshot'] += t2
            ...
            if ret_flag == -1:
                ...
                next_channel.queue_declare(queue=config.MQ_QUEUE_3RD, durable=True)
                next_channel.basic_publish(exchange='',
                                           routing_key=config.MQ_QUEUE_3RD,
...
def redeliver_message(message):
    request_url = config.WEB_SERVER_URL + '/api/redeliver?routing=' + config.MQ_QUEUE_2ND
...
channel.basic_consume(callback, queue=config.MQ_QUEUE_2ND)
